package main.java.code.TicTacToe.src.cs3500.tictactoe.model;
...
public class TTTModelImpl implements TicTacToeModel {
  ...
  private TTTModelImpl(int width, int height, int goal) {
    if (goal > height && goal > width) throw new IllegalArgumentException("Unwinnable game");
    this.width = width;
    this.height = height;
    this.goal = goal;
  ...
  private void updateWinner() {
    ...
    if (this.moveCount == this.getWidth() * this.getHeight())
